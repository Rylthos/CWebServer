...
void send_file(struct sockaddr_in* srcAddr, struct sockaddr_in* destAddr, int seq_num, int ack_seq,
    int serverFD, char* filename, ssize_t length)
{
    FILE* file;
    ...
    if (parse_file_ext(filename, length, &fileBuf, &header)) { // Error occured
        send_msg(srcAddr, destAddr, seq_num, ack_seq, serverFD, (uint8_t*)fileBuf, strlen(fileBuf));
        return;
    }
...
void handle_get(struct sockaddr_in* srcAddr, struct sockaddr_in* destAddr, int seq_num, int ack_seq,
    int serverFD, uint8_t* buf, ssize_t buf_length)
{
    if (buf[0] == '/') {
        send_file(srcAddr, destAddr, seq_num, ack_seq, serverFD, (char*)buf, buf_length);
    }
}
